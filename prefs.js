...
export default class HotspotToggleprefs extends ExtensionPreferences {
    fillPreferencesWindow(window) {
        ...
        const pskRow = new Adw.SwitchRow({
            title: _('Raw PSK'),
            subtitle: _('Treat password above as a 64 hex-digit pre-shared key instead of a passphrase (create_ap --psk)'),
        });
        ...
        const deviceGroup = new Adw.PreferencesGroup({
            title: _('Device'),
            description: _('Leave "Internet from" set to the same WiFi card (or empty) to keep using WiFi as a client while hosting the hotspot on the same card — that\'s create_ap\'s virtual-interface mode.'),
        });
        ...
        const internetRow = new Adw.ComboRow({
            ...
            model: Gtk.StringList.new(['(same WiFi card — concurrent mode)', 'none (no internet sharing)', ...allIfaces]),
        });
        {
        ...
        const applyGroup = new Adw.PreferencesGroup({
            description: _('Changes here take effect next time you toggle the hotspot on. Use "Apply Now" to restart it immediately with the new settings.'),
        });
        page.add(applyGroup);
    ...
    _toast(window, text) {
        if (window.add_toast)
            window.add_toast(new Adw.Toast({title: text, timeout: 4}));
    }
    ...
    async _showQrDialog(window, settings) {
        ...
        if (pngBytes) {
            ...
            const label = new Gtk.Label({
                label: _('Install "qrencode" to generate a scannable QR code.\nShowing the raw WiFi string instead:'),
                wrap: true,
            });
            box.append(label);
